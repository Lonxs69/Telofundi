...
// ✅ OBTENER PERFIL DEL USUARIO AUTENTICADO - ✅ ACTUALIZADO CON SISTEMA SIMPLIFICADO
const getUserProfile = catchAsync(async (req, res) => {
  ...
  const userResponse = {
    ...
    // ✅ SIMPLIFICADO: Agregar datos de cliente con puntos y límites
    ...(user.userType === 'CLIENT' && {
      client: {
        ...user.client,
        points: pointsData,
        limits: limitsData,
        // ✅ NUEVO: Información del sistema simplificado
        systemInfo: {
...
// ✅ SUBIR FOTO DE PERFIL - COMPLETAMENTE CORREGIDO PARA MANEJAR TIMEOUTS
const uploadProfilePicture = catchAsync(async (req, res) => {
  const userId = req.user.id;
  
  console.log('📸 === UPLOAD PROFILE PICTURE CONTROLLER ===');
  console.log('📸 User ID:', userId);
  console.log('📸 Has file:', !!req.file);
  console.log('📸 Has uploadedFile:', !!req.uploadedFile);
  console.log('📸 Has uploadError:', !!req.uploadError);

  // ✅ VERIFICAR SI HAY ERROR DE UPLOAD DESDE EL MIDDLEWARE
  if (req.uploadError) {
    console.error('❌ Upload error from middleware:', req.uploadError);
    
    // ✅ DETERMINAR CÓDIGO DE ERROR ESPECÍFICO
    let errorCode = 'UPLOAD_ERROR';
    let statusCode = 500;
    
    if (req.uploadError.includes('tardó demasiado') || req.uploadError.includes('timeout')) {
      errorCode = 'UPLOAD_TIMEOUT';
      statusCode = 408; // Request Timeout
    } else if (req.uploadError.includes('demasiado grande') || req.uploadError.includes('3MB')) {
      errorCode = 'FILE_TOO_LARGE';
      statusCode = 413; // Payload Too Large
    } else if (req.uploadError.includes('inválido') || req.uploadError.includes('corrupto')) {
    ...
    return res.status(statusCode).json({
      ...
      suggestion: statusCode === 408 
        ? 'Intenta con una imagen más pequeña o verifica tu conexión a internet'
        : statusCode === 413 
        ? 'Reduce el tamaño de la imagen a menos de 3MB'
  ...
  // ✅ VERIFICAR QUE EL UPLOAD A CLOUDINARY FUE EXITOSO
  if (!req.uploadedFile) {
    console.error('❌ No uploaded file result');
    return res.status(500).json({
      success: false,
      message: 'Error procesando la imagen. El archivo no se pudo subir al servicio de imágenes.',
      errorCode: 'UPLOAD_PROCESSING_ERROR',
      suggestion: 'Verifica que la imagen no esté corrupta e inténtalo de nuevo',
      timestamp: new Date().toISOString()
    });
  }
...
// ✅ OBTENER PERFIL POR ID - ✅ ACTUALIZADO para mostrar estado premium simplificado
const getUserById = catchAsync(async (req, res) => {
  ...
  const user = await prisma.user.findUnique({
    ...
    select: {
      ...
      // ✅ SIMPLIFICADO: Incluir datos de cliente para mostrar estado premium
      client: {
        select: {
          isPremium: true,
          premiumTier: true,
  ...
  // ✅ SIMPLIFICADO: Mostrar estado premium básico si es cliente
  if (user.userType === 'CLIENT' && user.client) {
    const now = new Date();
...
const getUserStats = catchAsync(async (req, res) => {
  ...
  try {
    ...
    // ✅ SIMPLIFICADO: Estadísticas de cliente con sistema de puntos
