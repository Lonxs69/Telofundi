...
// ✅ AGREGADAS LAS NUEVAS FUNCIONES PARA PUNTOS

// Obtener paquetes de puntos
...
const createPointsPaymentIntent = catchAsync(async (req, res) => {
  ...
  // Obtener paquete
  const package = await prisma.pointsPackage.findUnique({
  ...
  const paymentIntent = await stripe.paymentIntents.create({
    ...
    metadata: {
      ...
      basePoints: package.points.toString(),
      bonusPoints: package.bonus.toString(),
      totalPoints: totalPoints.toString(),
      userType: req.user.userType
    },
    description: `Compra de ${totalPoints} TeloPoints (${package.name})`,
    automatic_payment_methods: {
      enabled: true
    }
  });
  ...
  const payment = await prisma.payment.create({
    data: {
      ...
      status: 'PENDING',
      type: 'POINTS',
      description: `${package.name} - ${totalPoints} TeloPoints`,
      stripePaymentId: paymentIntent.id,
      clientId: req.user.client.id,
      metadata: {
        packageId,
...
const confirmPointsPayment = catchAsync(async (req, res) => {
  ...
  // Transacción para actualizar puntos y pago
  const result = await prisma.$transaction(async (tx) => {
    ...
    await tx.pointTransaction.create({
      data: {
        ...
        amount: totalPoints,
        type: 'PURCHASE',
        description: `Compra de ${payment.metadata.packageName}`,
        balanceBefore: updatedClient.points - totalPoints,
  ...
  res.status(200).json({
    ...
    data: {
      ...
      newBalance: result.points,
      package: {
        name: payment.metadata.packageName,
        basePoints: payment.metadata.basePoints,
        bonusPoints: payment.metadata.bonusPoints
      }
    },
