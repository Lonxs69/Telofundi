...
// ?? FIX CRÍTICO: WEBHOOKS DE STRIPE - DEBE IR PRIMERO ANTES DE CUALQUIER BODY PARSER
app.post('/api/payments/webhook/stripe', express.raw({ type: 'application/json' }), (req, res) => {
  console.log('?? Webhook de Stripe recibido - bypassing auth');
  const paymentController = require('./controllers/paymentController');
  paymentController.handleStripeWebhook(req, res);
});

app.post('/api/payments/webhook/points', express.raw({ type: 'application/json' }), (req, res) => {
  console.log('?? Webhook de Points recibido - bypassing auth');
  const paymentController = require('./controllers/paymentController');
  paymentController.handlePointsWebhook(req, res);
});
...
// ? MIDDLEWARE DE DEBUG MEJORADO Y REORGANIZADO
...
console.log('?? RATE LIMITING COMPLETAMENTE DESACTIVADO');
