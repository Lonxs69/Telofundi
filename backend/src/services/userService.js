/**
 * ====================================================================
 * 👤 USER SERVICE - GESTIÓN DE USUARIOS CON TELOPOINTS SIMPLIFICADO
 * ====================================================================
...
// ============================================================================
// CONFIGURACIONES DE LÍMITES - ✅ SIMPLIFICADO
// ============================================================================
...
const DEFAULT_LIMITS = {
  BASIC: {
    ...
    canAccessPremiumProfiles: false,  // ✅ Función Premium 1
    prioritySupport: false,
    canSeeOnlineStatus: false
  },
  PREMIUM: {
    // ✅ SIMPLIFICADO - Solo las 2 funciones premium + algunas básicas
    dailyMessageLimit: 50,
    maxFavorites: 25,
    canViewPhoneNumbers: true,
    canSendImages: true,
    canSendVoiceMessages: false,
    canAccessPremiumProfiles: true,   // ✅ Función Premium 1: Acceso posts premium
    prioritySupport: false,
    canSeeOnlineStatus: true
    // ✅ Chat prioritario se maneja por separado en chatPriorityUntil
  },
  VIP: {
    // ✅ MANTENER VIP por compatibilidad (aunque no se use actualmente)
...
const updateClientLimits = async (clientId, newLimits) => {
  try {
    const validFields = [
      ...
      'canAccessPremiumProfiles', // ✅ Función Premium 1
      'prioritySupport',
      'canSeeOnlineStatus'
    ];
...
/**
 * Obtener límites actuales del cliente - ✅ ACTUALIZADO para incluir chatPriorityUntil
 */
  ...
  try {
    const client = await prisma.client.findUnique({
      ...
      select: {
        ...
        canSendImages: true,
        canSendVoiceMessages: true,
        canAccessPremiumProfiles: true, // ✅ Función Premium 1
        prioritySupport: true,
        canSeeOnlineStatus: true,
        isPremium: true,
        premiumTier: true,
        premiumUntil: true,
        chatPriorityUntil: true, // ✅ Chat priority (separado o como parte de premium)
    ...
    // ✅ Verificar estado de chat priority (separado o premium)
    const chatPriorityActive = client.chatPriorityUntil && client.chatPriorityUntil > now;
    
    // ✅ SIMPLIFICADO: Premium activo = tiene las 2 funciones
    ...
    return {
      ...
      permissions: {
        ...
        canAccessPremiumProfiles: client.canAccessPremiumProfiles, // ✅ Función Premium 1
      ...
      premium: {
        ...
        // ✅ SIMPLIFICADO: Cuando premium está activo, tiene las 2 funciones
        functions: premiumActive ? {
          canAccessPremiumProfiles: true,  // ✅ Función 1
          chatPriorityActive: true         // ✅ Función 2
        } : {
          canAccessPremiumProfiles: false,
          chatPriorityActive: false
        }
      },
      // ✅ Chat priority (puede ser separado o parte de premium)
      chatPriority: {
...
/**
 * ✅ SIMPLIFICADO: Aplicar beneficios premium a un cliente (solo 2 funciones)
 */
const applyPremiumBenefits = async (clientId, tier, duration = null) => {
  try {
    ...
    // ✅ SIMPLIFICADO - Solo aplicar las 2 funciones premium
    const benefits = {
      canAccessPremiumProfiles: true,  // ✅ Función 1: Acceso posts premium
      // Chat priority se maneja por separado en chatPriorityUntil
      // Mantener algunos beneficios básicos de DEFAULT_LIMITS[tier] por compatibilidad
      dailyMessageLimit: DEFAULT_LIMITS[tier].dailyMessageLimit,
      maxFavorites: DEFAULT_LIMITS[tier].maxFavorites,
      canViewPhoneNumbers: DEFAULT_LIMITS[tier].canViewPhoneNumbers,
      canSendImages: DEFAULT_LIMITS[tier].canSendImages,
      canSeeOnlineStatus: DEFAULT_LIMITS[tier].canSeeOnlineStatus
    };
    ...
    if (duration) {
      ...
      // ✅ NUEVO: Si es premium, también activar chat priority por el mismo tiempo
      updateData.chatPriorityUntil = expirationDate;
    }
    ...
    logger.info('Premium benefits applied (2 functions)', {
      clientId,
      tier,
      duration,
...
/**
 * ✅ SIMPLIFICADO: Remover beneficios premium (revertir a BASIC)
 */
  ...
  try {
    ...
    const updatedClient = await prisma.client.update({
      ...
      data: {
        ...
        premiumTier: 'BASIC',
        premiumUntil: null,
        chatPriorityUntil: null // ✅ También quitar chat priority al quitar premium
      }
    });

    logger.info('Premium benefits removed, reverted to BASIC (2 functions removed)', { clientId });
    return updatedClient;
  } catch (error) {
...
/**
 * ✅ SIMPLIFICADO: Verificar si el usuario puede realizar una acción
 */
    ...
    switch (action) {
      case 'send_message':
        ...
        return { 
          ...
          canUsePoints: true,
          upgradeOptions: ['premium_1_day', 'premium_3_days', 'premium_1_week', 'premium_1_month']
        };
      ...
      case 'send_voice':
        ...
        return { 
          ...
          canUsePoints: false // VIP no disponible con puntos en esta configuración
        };
      ...
      // ✅ SIMPLIFICADO: Acceso a posts premium (Función 1)
      ...
      // ✅ Chat priority (separado o parte de premium)
      case 'chat_priority':
        if (limits.chatPriority.isActive) {
          return { 
            allowed: true, 
            message: 'Ya tienes prioridad en chat activa',
            expiresAt: limits.chatPriority.expiresAt,
            source: limits.chatPriority.source
          };
        }
        return {
          allowed: true,
...
/**
 * ✅ SIMPLIFICADO: Procesar uso de puntos para acción específica
 */
const usePointsForAction = async (userId, action, targetData = {}) => {
  try {
    ...
    // ✅ SIMPLIFICADO: Solo acciones permitidas
    const validActions = [
      'chat_priority',      // Chat priority separado
      'premium_1_day',      // Premium con 2 funciones
      'premium_3_days',     // Premium con 2 funciones
      'premium_1_week',     // Premium con 2 funciones
      'premium_1_month'     // Premium con 2 funciones
    ];
    ...
    if (action === 'chat_priority') {
      // Usar la función específica de chat priority
      result = await pointsService.activateChatPriority(clientId);
      ...
      // ✅ SIMPLIFICADO: Usar la función de premium (que incluye las 2 funciones)
      result = await pointsService.activatePremiumWithPoints(clientId, action);
