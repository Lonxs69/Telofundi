...
const handleGoogleCallback = (req, res, next) => {
  passport.authenticate('google', { session: false }, (err, user, info) => {
    ...
    if (err) {
      ...
      const redirectUrl = `${frontendUrl}${config.auth.errorRedirectPath}${errorMessage}`;
    ...
    try {
      ...
      const redirectUrl = `${frontendUrl}${config.auth.successRedirectPath}${queryParams.toString()}`;
      
      logger.info('✅ Google OAuth completado exitosamente:', {
        userId: user.id,
        username: user.username,
        userType: user.userType,
        redirectUrl: `${frontendUrl}${config.auth.successRedirectPath}auth=success`
      });
