...
const options = {
  definition: {
    ...
    servers: [
      {
        url: config.urls.api,
        description: config.environment.isProduction ? 'Servidor de Producción' : 'Servidor de Desarrollo'
      }
    ],
    components: {
      securitySchemes: {
...
const setupSwagger = (app) => {
  ...
  const swaggerUiOptions = {
    customCss: `
      ...
      ${config.environment.isProduction ? '.swagger-ui .scheme-container { display: none }' : ''}
    `,
    ...
    swaggerOptions: {
      ...
      filter: true,
      tryItOutEnabled: !config.environment.isProduction // ✅ Deshabilitar "Try it out" en producción
    }
  };
  ...
  // ✅ URL DINÁMICA EN EL LOG
  const baseUrl = config.urls.api.replace('/api', '');
